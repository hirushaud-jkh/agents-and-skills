...
- Call `#tool:mcp__azure-devops__wit_my_work_items` to identify the current user (name + email).
- Store in session state. Never ask the user for their own name.

### Step 2 — Select Project & Team
- Call `#tool:mcp__azure-devops__core_list_projects` and present the list.
- Once the user picks a project, call `#tool:mcp__azure-devops__core_list_project_teams`.
...
Call `#tool:mcp__azure-devops__wit_get_work_item_type` for common parent types in order: `"User Story"`, `"Feature"`, `"Epic"`. Use the first one that exists in the project as the **parent type** (grouping container).
Then determine the **time-bearing type** (where hours are logged): call `#tool:mcp__azure-devops__wit_get_work_item_type` for `"Task"`. If it doesn't exist, use the same type as the parent (some projects log time directly on the parent).
...
Check whether `Custom.ActualEndDate` exists by calling `#tool:mcp__azure-devops__wit_get_work_item_type` and scanning the fields list. Set `has-actual-end-date: yes/no` in the profile.
...
- Call `#tool:mcp__azure-devops__work_list_team_iterations` for the selected team.
...
- Call `#tool:mcp__azure-devops__wit_get_work_items_for_iteration` and filter by `System.AssignedTo` = current user.
...
Use `#tool:mcp__azure-devops__wit_create_work_item` with `workItemType: profile.parent-type`.
...
Use `#tool:mcp__azure-devops__wit_add_child_work_items` with `parentId` = the parent item ID and `workItemType: profile.time-type`.
...
Use `#tool:mcp__azure-devops__wit_work_items_link` with `type: "parent"` to link each child to its parent.
Do NOT use `System.Parent` via field update — it does not work.

### Step D — Set Time, State, and Dates
Apply to `profile.time-type` items (or parent items if flat project).

Use `#tool:mcp__azure-devops__wit_update_work_items_batch` for 3+ items, `#tool:mcp__azure-devops__wit_update_work_item` for fewer.
...
1. Fetch current state with `#tool:mcp__azure-devops__wit_get_work_item` before changing anything (for the ledger).
2. Apply changes via update tools.
3. Never touch items not assigned to the current user.

### Field Errors
If a field update fails, call `#tool:mcp__azure-devops__wit_get_work_item_type` for that work item type to check available fields, then retry with the correct field path.
